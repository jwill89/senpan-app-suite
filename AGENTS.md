...
│       │   └── admin/                ← AdminSidebar + one component per tab + modals (CardPreview, EndGame, WinnerVerify, HalftimePrompt) + ThemeColorPickerTool. Tabs: Game, Cards, WinnersLog, Categories, NewPattern, EditPatterns, Presets, RaffleForm, Open/ClosedRaffles, Announcements, BookClub (one generic tab serves every club), Settings, Themes, Fonts, CarrdUpload
...
│       │   ├── store.go              ← Store struct + core bingo/card/pattern/raffle/settings/style/preset methods
...
- Admin: `/admin` (layout, `requiresAdmin`) with child routes grouped into five
  sidebar sections — **bingo** (`game,cards,winners-log,categories,new-pattern,patterns,presets`),
  **raffles** (`new,open,closed`), **teahouse** ("Senpan Tea House":
  `teahouse/announcements` + one `bookclub/<slug>` route per registered club),
  **system** (`settings,themes`), and **atelier** (`fonts,carrd`). The admin tabs
  are **child routes** of the `AdminView` layout, so the sidebar/topbar persist
  while the matched child renders the active tab. The per-club book-club routes
  are generated from the `BOOK_CLUBS` registry (`lib/constants.ts`) and all render
  the single generic `BookClubTab.vue`.
...
- Test files are colocated as `src/**/*.test.ts`. Current coverage: `lib/{constants,api,endpoints,exportCard,theme,datetime,freshness}.test.ts`, `stores/{player,ui,raffles}.test.ts`, `components/common/BingoBoard.test.ts`.
...
**Admin sections** (`adminSection`, sidebar highlight): `bingo` | `raffles` | `teahouse` | `system` | `atelier`.
**Admin tabs** (`adminTab` / route): `bingo-game` · `bingo-cards` · `bingo-winners-log` · `bingo-categories` · `bingo-new-pattern` · `bingo-patterns` · `bingo-presets` · `raffle-new` · `raffle-open` · `raffle-closed` · `teahouse-announcements` · `bookclub-<slug>` (one per registered club, e.g. `bookclub-yaoi`, `bookclub-yuri`) · `system-settings` · `system-themes` · `atelier-fonts` · `atelier-carrd`.
