...
export const STAMP_COLORS: StampColor[] = [
  ...
  { id: 'highlight', name: 'Highlight', value: 'rgba(255,216,3,.55)' },
...
/** An empty 5x5 number board (all 0; centre [2][2] is the FREE space). */
export function emptyNumberBoard(): number[][] {
  return Array.from({ length: 5 }, () => Array.from({ length: 5 }, () => 0))
}

/**
 * Generates a random valid 5x5 bingo board (each column shuffled from its range;
 ...
 * `bingo.ValidateBoard`): 5x5, FREE centre, each column within its range with no
...
export function validateBoard(board: number[][]): string {
  if (board.length !== 5 || board.some((r) => r.length !== 5)) return 'Card must be a 5x5 grid.'
...
/** Creates an empty 5x5 boolean grid for the pattern editor. */
