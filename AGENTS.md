...
C#/.NET 10 + ImGui, published as _Senpan Admin Companion_) - is a **second admin
...
coder/websocket - per-user accounts (argon2id) + WebAuthn passkeys + optional
Cloudflare Turnstile + per-page permissions - embedded tzdata + background
schedulers (Discord embeds) - Vitest + Vue Test Utils - C#/.NET 10 Dalamud plugin
(ImGui) - GitHub Actions CI.
...
password (argon2id hash, `internal/auth`) **or a passwordless WebAuthn passkey**,
are created **inactive** via a hidden registration page, and an admin activates
them and grants **per-page permissions** (one key per admin page). Admins
implicitly hold every permission. A password change/reset invalidates the
account's other sessions (a `password_epoch` checked each request). A bootstrap
`admin`/`admin` account is seeded by migration v22 and **must be rotated
immediately**. See **Authentication & authorization** below.
...
|           +-- announcements.go      <- announcement types + roles + announcements (send_now/skip), optional role tag + location, image upload/list, recurrence math + RunAnnouncementScheduler
...
- **Password changes invalidate other sessions.** Each account carries a `password_epoch`; a change (self-service `/api/account`) or admin reset bumps it (`SetUserPassword` does so in one statement). Every cookie session records the epoch it was minted with, and `currentUser`/`wsSessionUser` reject a session whose stored epoch no longer matches - so a password change logs out **every other** browser session, while the initiating session is re-stamped so it stays valid. The change also revokes the account's personal-access token.
- **Real client IP for throttling.** The login/registration/passkey rate limiter keys on the same authoritative client IP as the access log: `CF-Connecting-IP`/`X-Forwarded-For` are trusted **only** when the immediate peer is the loopback reverse proxy, otherwise `RemoteAddr` is used - so behind Cloudflare users don't share (or forge) a throttle bucket. Passkey-login `begin` shares this limiter too.
- **Passkeys (WebAuthn).** Accounts can also log in **passwordless** via discoverable WebAuthn credentials (`go-webauthn`, `server/passkeys.go` + `store/passkeys.go` + `user_passkeys`), offered alongside password login. A logged-in account self-registers one or more passkeys (`POST /api/account/passkeys/register/begin|finish`, `GET`/`DELETE /api/account/passkeys[/{id}]`); login (`POST /api/auth/passkey/begin|finish`) resolves the account from an 8-byte user-ID handle, requires an active account, verifies the assertion (with signature-counter clone detection), and establishes the same SCS session (token rotated). The RP-ID/origin derive from the request host at runtime (no env), and the challenge is held in the session between begin/finish. Frontend: `lib/passkeys.ts` + `stores/auth.ts`, surfaced on `AdminLoginView` and a manage-passkeys modal in `AdminView`.
- **Bot check (Cloudflare Turnstile).** When a Turnstile secret **and** sitekey are configured (`-turnstile-secret`/`-turnstile-sitekey` flags or `APPSUITE_TURNSTILE_SECRET`/`_SITEKEY` env), the public POST paths - login, registration, raffle sign-up, custom card request - verify a one-time token server-side (`server/turnstile.go`, **fail-closed**: an empty/invalid token or a transport error is rejected) before doing credential/write work; a failed challenge returns **403**. Unconfigured (empty secret) disables the check entirely (logged at startup), and the public sitekey is served via `GET /api/config` (empty when off) so the frontend widget (`TurnstileWidget.vue`) skips rendering. A failed login challenge is deliberately **not** counted against the rate limiter (so an expired token can't lock out a real user).
...
**Discord embeds**: `embeds.go` holds the shared embed schema, a fluent `newEmbed()...build()` builder (auto-truncates to Discord's per-field limits, skips empty fields), a `#rrggbb`->int colour helper, and `postDiscordEmbed()` / `postDiscordWebhook()` transport (the latter also carries an optional `content` mention + `allowed_mentions`). Feature code (announcements, reading-list items) only assembles a builder chain - no feature has its own transport. **Dev safety:** because a development server is normally pointed at a COPY OF THE LIVE DATABASE - whose announcement types, book clubs, affiliates and tea rooms all carry real webhook URLs - two valves sit at that same funnel (`Server.SetWebhookPolicy`, wired from `-webhook-dry-run` / `APPSUITE_WEBHOOK_DRY_RUN=1` and `-webhook-override` / `APPSUITE_WEBHOOK_OVERRIDE`). Dry run suppresses every outbound post and logs it; override redirects every post to one test webhook whatever the database says; dry run wins when both are set, both are off in production, and both log a WARN at startup. Put the override URL in the environment, never in the repo - it is a credential. Without one of these, simply starting a local server is enough for the announcement scheduler to post to production channels. Outbound calls (AniList + webhooks) share `bookclubHTTPClient` (15s timeout).

**Background scheduler**: `main.go` launches `RunAnnouncementScheduler` in a goroutine tied to a shutdown-cancelled context. It uses the ticker/sweep loop in `scheduler.go` (`runScheduler`): ticks every 30s (and sweeps once on startup to catch up after downtime), posts due announcements to their type's webhook, and is resilient - an announcement whose webhook is unset is left pending, a failed post is retried next tick, and `skip_count`/recurrence advance the cursor without losing the schedule. `skip_count` is how many upcoming occurrences to pass over (each skipped one decrements it), so a Friday+Saturday announcement can sit out a whole weekend in one go; it is schedule-cursor state, written ONLY by the skip endpoint and the scheduler - `UpdateAnnouncement` deliberately leaves it alone, since writing it from the form is what used to clear a pending skip whenever an admin saved an unrelated edit. Wall-clock times are resolved against each announcement's IANA timezone (DST-safe) and stored as UTC; `tzdata.go` embeds the zone database so this works on Windows hosts too.
...
**What gets an Info line.** Beyond per-request logging: every **delivered** Discord post (`discord post sent`, emitted once at the `postDiscordWebhook` funnel with a `webhookTarget` label - kind + record name - so a new posting feature is covered without adding a line); **scheduler** start/stop for both background loops; the **bingo game** lifecycle (`bingo game started` / `bingo game ended` / `bingo halftime decision`, with the acting admin via `Server.actorName`); a consumed **skip** on an announcement (with the count remaining); and manual announcement sends + reading-list publishes with their `by` attribute. Two things are deliberately quieter: a **failed** post is logged by the caller that knows whether it retries or returns a 502 (never at the funnel too), and high-frequency events stay at **Debug** or are dropped entirely - individual scheduler sweeps (2,880/day at a 30s tick) and individual bingo draws (~75/game) would bury everything else. When adding a log line, ask what it costs at the event's real frequency.
...
**Bingo card format**: 5x5 array `board[row][col]`; col 0=B(1-15) ... col 4=O(61-75); centre `[2][2] = 0` (FREE).
**Pattern format**: 5x5 `[][]bool` grid; `true` = cell must be called for win; centre always counts.
**Card IDs**: 6-char alphanumeric (no ambiguous chars 0/O/1/I/l), enforced unique in DB.

## Design philosophy

The principles below are _why_ the conventions exist - keep changes aligned with them.
...
| `cards`                 | `id TEXT PK`, `board_data TEXT` (JSON 5x5 array), `player_name TEXT`, `details TEXT`, `world TEXT`, `protected INTEGER` (spared by Delete-All), `custom_status TEXT` (`''` normal / `pending` / `approved` - the custom-card-request lifecycle), `created_at`                                                                                                                                                                                                          |
...
| `settings`              | `key TEXT PK`, `value TEXT` - key-value config (e.g. `game_details`, `active_style_id`, `app_title`, `default_draw_delay`, `frequent_winner_threshold`/`_hours`, `header_font`, `google_fonts_api_key`, `anilist_api_url`, `bingo_join_prompt`, `yoever_cooldown_seconds`, `custom_card_cost`, per-club reading-list `discord_webhook_url_<slug>`, and the secret `tearoom_webhook_url`) - note Turnstile keys are **not** here; they come from flags/env              |
...
| `stamp_rallies`         | `id INTEGER PK`, `title`, `card_image`, `not_stamped_image`, `available_from`/`available_to`, `details`, `redeem_instructions`, `status` (open/closed), `public_signup` (self-service sign-up opt-in, default 0), `created_at` - a stamp-rally event (Festival -> Stamp Rally)                                                                                                                                                                                                                                                   |
...
cards,winners-log,patterns,presets}`), **teahouse** ("Senpan Tea House":
  `teahouse/announcements`, `teahouse/affiliates`, `teahouse/tea-rooms`, and one
  `teahouse/bookclub/<slug>` route per registered club), **festival** (`festival/{garapon,stamp-rally}`; the
...
- Markdown **authoring** -> `MarkdownEditor.vue` wraps **Milkdown (Crepe)** (`@milkdown/crepe`), a WYSIWYG editor built via Crepe's tree-shakable `CrepeBuilder` (only the features used are imported, dropping the CodeMirror/LaTeX bundles). It's still a sizeable bundle, so both the library **and** its CSS are dynamic-imported on mount (kept out of the initial load); it maps its colours onto the app theme variables, is limited to a Discord-safe subset (inline formatting + headings/quotes/lists/dividers/links), and emits **markdown** (what we store and what Discord renders). Formatting lives on a persistent **top bar** (Crepe's `topBar`, not the floating selection toolbar); `buildTopBar` prunes the buttons for features the editor doesn't enable (image/table/code-block/math) plus task lists, and reuses the emptied "block" group for an **emoji button** - an empty group would still render its divider. The emoji picker inserts the literal character, never a `:shortcode:`, which the markdown serializer would escape. Reused by raffles, book-club items, and announcements.
...
**Public routes**: `/` - `/play/:cardId` - `/raffles` - `/raffles/:id` - `/garapon/:token` (token-gated) - `/stamp-card/:token` (token-gated) - `/card-requests` (submit a self-designed bingo card for admin approval) - `/admin/login` - `/admin/register` (hidden).
**Admin sections** (`adminSection`, sidebar highlight): `bingo` | `teahouse` | `festival` | `atelier` | `system`.
**Admin tabs** (`adminTab` / route): `bingo-game` - `bingo-cards` - `bingo-winners-log` - `bingo-patterns` - `bingo-presets` - `teahouse-announcements` - `teahouse-affiliates` - `teahouse-tea-rooms` - `bookclub-<slug>` (one per registered club, e.g. `bookclub-yaoi`, `bookclub-yuri`) - `festival-garapon` - `festival-stamp-rally` - `teahouse-raffles` (shown in the Festival section, id unchanged) - `system-settings` - `system-themes` - `system-images` - `system-users` (admin-only) - `system-logs` (admin-only) - `atelier-fonts` - `atelier-carrd`. Each tab id (except `system-users`/`system-logs`) doubles as its **page-permission key**.
...
Enforced by `npm run lint:conventions` (`frontend/scripts/check-conventions.mjs`), which
runs in `scripts/check.ps1` and CI. ESLint does not lint CSS and Prettier only formats
it, so without that script none of this is checked.

**The two tiers.** Shared styles are the default; a `<style scoped>` block is the
exception and needs a reason. Scoped CSS is for what is genuinely one-of-a-kind about
_this_ component - its layout and its own bespoke pieces. Anything that describes a
_kind of thing_ (a card, a dropzone, a row, a badge, a form control) belongs in
`assets/styles/` under a role name, even the first time it is written.

The promotion trigger is **the second occurrence**: the moment a rule is about to be
written a second time, in any file, it becomes a shared object instead. Do not wait for
a third. Copying a block between two components and renaming its prefix is the specific
move that produced ~980 duplicated lines and the media-manager, sub-panel, link-button
and inline-checkbox clones that had to be unpicked later.

Scoped CSS is not free: it compiles to `[data-v-xxx]`, which raises specificity to
(0,2,0) and quietly outranks the global object it duplicates, so the two drift without
anything failing. Since the compiler already guarantees uniqueness there, the naming
rules in a scoped block exist for _readability_, not collision safety - the discipline
differs by layer:

| Layer  | Where                                                               | Rule                                                           |
| ------ | ------------------------------------------------------------------- | -------------------------------------------------------------- |
| Global | `assets/styles/*.css`, loaded in the cascade order set by `app.css` | Full discipline: shared objects, parts, modifiers, state       |
| Scoped | `<style scoped>` in 47 SFCs                                         | The component's name as prefix, plain kebab-case, nothing more |

Global is currently ~4,190 lines and scoped ~2,860 across 47 SFCs (40 other SFCs carry
none). Scoped total should trend **down**; a change that grows it wants a look.

**Naming.**

1. **Shared objects are role-named with no prefix** and live in `utilities.css` /
   `components.css`: `.card`, `.badge`, `.chip`, `.field`, `.list-row`, `.data-table`.
2. **A part joins to its parent with `-`**: `.list-row-title`, `.empty-state-icon`,
   `.data-table-wrap`.
3. **A modifier uses `--`**: `.badge--accent`, `.home-card--dest`,
   `.list-row-meta--inline`.
4. **State always uses `is-`**: `.is-active`, `.is-open`, `.is-selected`, `.is-free`.
   Never a bare state word - `active` and `disabled` also occur as data values
   (`status === 'active'`) and as HTML attributes, so the bare form is ambiguous to
   both a reader and a find-and-replace.
5. **A scoped component's classes carry its full name**, spelled out:
   `.announcement-badge`, `.changelog-rail-item`, `.theme-preview-cell`. Not
   abbreviations - `ann-`, `cl-`, `tp-` were all retired, because the prefix's only
   job is telling a reader where the class lives.
6. **Never name the appearance; name the role, or the token it applies.**
   `.text-muted` applies `--text-muted`; `.text-highlight` applies `--highlight`.
   This is the rule the retired `gold` broke: `.text-gold` described a colour, so
   renaming the token to `--highlight` left the class describing something untrue.
7. **BEM's `__` is not used.** It exists to manufacture uniqueness in a global
   namespace, which `<style scoped>` already provides; only 6 of 47 components ever
   adopted it and those read the worst.
...
does not exist. The checker fails on a class that only ever appears as the head of a
compound selector. Exempt: `is-*` state, and co-class modifiers - a class that always
shares its `class` attribute with another (`class="opacity-slider sound-volume"`)
modifies _that_ object and rightly has no base. This rule exists because a bulk
rule-dropper matching only `^\.name {` was once run over all of `src/` and stripped the
base rules out of `utilities.css`, leaving their `:hover` compounds behind - the file
still looked populated, every test passed, and the objects rendered unstyled.

**Scoped CSS may only shrink.** `frontend/scripts/scoped-css-baseline.json` records the
scoped line count of every SFC, and the checker fails when one grows. A file already in
the baseline is grandfathered at its current size - legacy bulk is tolerated, growth is
not. A file _not_ in the baseline is new work and may not exceed **120** scoped lines,
because new work has no legacy to plead. The total is pinned too, so rules cannot be
shuffled between files for free.

After a reduction, run `npm run lint:conventions:baseline` and commit the result. Going
_under_ baseline never fails - it prints a note - because a check that failed the moment
you improved something would only teach people to re-baseline without reading. This
exists because scoped CSS reached near-parity with the global stylesheets (3,360 vs
4,013) before anyone noticed, and unpicking it took days.

**A themeable token must have a consumer outside the theme editor.** The editor renders
every token by definition, so it cannot vouch for one; a token only it references is a
knob in the UI that changes nothing in the app. `--accent-hover` and `--accent-2-hover`
failed the other way - dropped from the stylesheets while still offered in the editor -
and stayed broken until this pass.

**No dead classes in markup.** A class hard-coded in a static `class="..."` must be
styled by something. The checker fails otherwise. Exempt: a class referenced from a
`.ts` file, which is a deliberate test or JS hook rather than dead markup
(`ChangelogModal.test.ts` queries `.changelog-entry`). Only static attributes are
checked - `:class` is a JS expression and cannot be parsed reliably enough to gate on.
This found twenty, and two of them were live defects rather than clutter: `mt-4` /
`mt-10` were never defined, so those elements got no margin at all, and `btn-secondary`
was never defined, so three Server Logs buttons rendered with no fill.

**Tokens.** `--radius`, `--radius-media`, `--body-font` and `--header-font` live in
`tokens.css` but are **not** themeable and must stay out of the themeable copies. Every other `:root` token must appear in all
three sources, which the checker verifies in both directions:

- `frontend/src/assets/styles/tokens.css` - the declarations themselves
- `frontend/src/lib/theme-tokens.ts` - the editor's allowlist + labels + defaults
- `backend/internal/store/styles.go` - `themeTokenOrder`, used to generate the `:root{}`

The checker also fails on any `var(--x)` that resolves to nothing, because an
undefined custom property silently falls back to `inherit` - invisible until someone
notices a wrong colour.
...
_Senpan Admin Companion_ (Dalamud InternalName/DLL `SenpanCompanionAdmin`; the
...
client<->client networking, so the server _must_ stay the relay - the plugin never
talks peer-to-peer.

- **Auth - personal access token.** The plugin sends `Authorization: Bearer pat_...`
  on REST and `?token=` on the WebSocket upgrade; the server accepts the `?token=`
  query fallback **only** on the WS upgrade (REST requires the header, keeping the
  token out of access logs/history/Referer). The server URL + token are entered via
  `/senpan config` (PAT support shipped in FE+BE v1.5.0) and the **token is encrypted
  at rest** with the Windows Data Protection API (DPAPI, CurrentUser scope, see
  `Services/TokenProtector.cs`) - the config JSON stores ciphertext, not the
  plaintext token; a pre-encryption plaintext token migrates on first load.
  Permissions are enforced server-side exactly as for the web admin (`bingo-cards` /
  `bingo-game` / `teahouse-raffles`). `/senpan` opens the main window.
...
# -webhook-dry-run is not optional in practice: devdata is a COPY OF THE LIVE
# database, so its announcement types, book clubs, affiliates and tea rooms hold
# REAL webhook URLs - without it, the announcement scheduler posts to production
# channels on its own, no clicking required. Swap it for
# -webhook-override <test-webhook-url> when you need to see a post actually land.
cd backend; go run . -addr :8080 -db ../devdata/database.sqlite -webroot ../devdata/webroot -webhook-dry-run
...
- **Which folder a new image goes in.** A consequence of that strip: `frontend/public/images/`
  is a **dev-time mirror of the server's persistent tree**, not a way to ship a file. Put an
  image there and it works on `npm run dev` and 404s on live until someone uploads it by hand -
  and because the browser caches the miss, it stays broken for a while after they do. So:
  admin-uploadable or operator-replaceable art (raffle prizes, the logo, PWA icons) belongs in
  `public/images/` **and** `deploy/images/`; art that ships with the code belongs in
  `src/assets/images/` as an `import` (Vite emits a content-hashed file into `dist/assets/`,
  which needs no deploy step and can never serve a stale cache entry). `KonamiEgg.vue` does
  the latter; `YoeverOverlay.vue` predates the rule and still reads `${BASE_URL}images/`.
...
- **CSS naming + token drift**: see **CSS conventions** under **Frontend** - role-named shared objects, `-` for a part, `--` for a modifier, `is-` for state, the component's full name as a scoped prefix, and never the appearance. Shared styles are the default; a rule becomes a shared object at its **second** occurrence. Scoped CSS may only shrink (ratcheted against `scoped-css-baseline.json`; new SFCs cap at 120 lines). The themeable token list is duplicated in three sources that must agree, and every themeable token must have a consumer outside the theme editor. All of it is gated by `npm run lint:conventions`.
...
- **Crash isolation**: a top-level panic-recovery wrapper in `ServeHTTP` logs the panic (with stack, path redacted) and returns a 500 while the access-log line still records; long-lived/background goroutines (the auto-draw loop, the WS client read/write pumps, the scheduler sweep, and the deferred player-draw/half-time timers) each `recover()` so one panic can't take down the process.
- **HTTP server timeouts**: `ReadHeaderTimeout`/`ReadTimeout`/`IdleTimeout` are set on the `http.Server` to bound slow-client (Slowloris) exposure; there is deliberately **no** global `WriteTimeout` (it would sever the long-lived `/api/ws` socket). A listen failure sends to an error channel that triggers the normal graceful shutdown (deferred cleanup runs) rather than `os.Exit`.
- **WS upgrade is access-logged**: because `/api/ws` bypasses the request-logging middleware, `handleWS` emits its own structured log line (method/path/status/ip/actor) - including rejected admin attempts - so the privileged channel is auditable.
...
- **Rate limiter**: IP-based brute-force protection for login / registration / passkey-login (begin + finish). Its per-IP key comes from the same `logClientIP` logic the access log uses - `CF-Connecting-IP`/`X-Forwarded-For` trusted **only** from the loopback proxy peer, else `RemoteAddr` - so a client behind Cloudflare can neither share nor forge another IP's throttle bucket (`clientIP` delegates to `logClientIP`).
...
- **Discord embeds (shared plumbing)**: every webhook-posting feature (announcements, reading-list items) builds its embed with the fluent `newEmbed()...build()` builder in `server/embeds.go` and posts via `postDiscordEmbed()` (or `postDiscordWebhook()` for a payload with an optional `content` mention + `allowed_mentions`). The builder auto-truncates to Discord's per-field limits, skips empty fields, and caps at 25 fields, so callers pass raw content. New embed shapes add a builder chain, not new transport. `postDiscordEmbed`/`postDiscordWebhook` take a `context.Context` (built via `http.NewRequestWithContext`), so a shutdown or client disconnect cancels an in-flight webhook POST. Times are emitted as Discord `<t:unix:...>` tokens so each viewer sees their own zone.
...
- **Background scheduler**: `RunAnnouncementScheduler` is launched as a goroutine in `main.go` against a shutdown-cancelled context, ticks every 30s, sweeps once on startup, and is crash-tolerant (unset webhook -> left pending; failed post -> retried; `skip_count`/recurrence advance the cursor). The due-row query is index-backed (`(active, next_post_at)`). It uses the ticker/sweep loop in `server/scheduler.go` (`runScheduler(ctx, name, interval, sweep)`) - the `Run...Scheduler` is just a one-line wrapper supplying its interval + sweep func.
...
- **AniList proxy**: `GET /api/bookclub/lookup?q=|id=` proxies the public AniList GraphQL API server-side (avoids browser CORS + centralizes field mapping); the endpoint is configurable via the `anilist_api_url` setting. That URL is validated against `isAllowedAniListURL` (https on an `anilist.co` host) **at save time and again at read time** (a disallowed stored value falls back to the default), and the dedicated `anilistHTTPClient` re-validates **every redirect hop** against the same allowlist - SSRF defense in depth so the outbound POST can't be pointed at an internal service or cloud-metadata host. `anilistToItem` maps a Media node to a reading-list item suggestion.
...
- **Admin UI primitives** (`components/common/ui/`): admin forms and tables are built from a small set of presentational components so every screen is consistent by construction - don't hand-roll `.field`/label/table markup. Use `AdminPanel` (the `.admin-panel` card; pass `title`+`icon` or supply a custom header slot), `FormField` (label + control slot + optional `help`/`#help`; controls auto-stretch full-width, so no `.field-input-full` on direct children), `FormRow` (equal-width side-by-side fields - for unequal widths use a plain `.flex-row` and pass `style="flex:..."` to each `FormField`, which falls through to its root), `FormActions` (button row; `align="start|end|between"`), `DataTable` (generic sortable table - `columns`/`rows`/`row-key`, `#cell-<key>` slots, `#empty` slot, optional `rowClass`. **It owns the row pipeline: filter -> sort -> paginate**, on TanStack Table **v9** (headless - row model only, no library markup or CSS). v9 composes the engine from explicit `tableFeatures({...})` rather than bundling everything: each capability is opted in and every row model (`createFilteredRowModel`, `createSortedRowModel`, `createPaginatedRowModel`, `createFacetedRowModel`, `createFacetedUniqueValues`) must be listed AFTER the feature it depends on; the core row model is implicit. The row type is `T extends Record<string, any>`, matching v9's own `RowData`. Pass the FULL list, never a page: sorting must run before pagination, or a header click reorders only the page on screen. Optional `filter`/`filter-fn`, `page-size` + `v-model:page`, `default-sort`; it reports `{ total, totalPages, facets }` via `@update:view` for the toolbar count, facet dropdowns + `PaginationBar`. A first click sorts ascending and sorting is locale-/numeric-aware. Opt-in extras, each off unless asked for: `resizable` (drag a column edge; declared widths and auto columns are untouched until one is actually dragged), `selectable` + `v-model:selected` (checkbox column; the header box selects every row matching the CURRENT filter, not just the page - bulk delete confirms once for the set), `group-by` (keeps rows sharing a value contiguous as a PRIMARY sort key, so column sorting still orders within each group), `column-filters` + a `facetable` column (exact-match facet filtering), and `exportCsv(name)` via a template ref (the filtered rows in sort order across ALL pages, RFC 4180 quoted, UTF-8 BOM; skips `noExport` columns); the trailing **actions column uses `label: ''`** (no header) **+ `align: 'right'`**, and its `#cell-actions` content is wrapped in **`.row-actions`** (flex, `gap`, right-justified) - match this for every table with row actions, never a bare inline span), `DataTableToolbar` (the ONE toolbar above a table - fixed order `[search filters] ... [per-page count actions]`, with `count`/`total`/`noun` driving a uniform count string; never hand-roll a `.flex-toolbar` above a `DataTable`, that is exactly how seven tables drifted apart), `PaginationBar`, and `EmptyState`. The CSS for these lives in `styles/utilities.css` (the "FORM & TABLE PRIMITIVES" and "MANAGER LAYOUT" blocks) and uses theme tokens so themes restyle them.
...
- **SVG sanitization**: uploaded theme/flourish SVGs are rendered **inline** (via `v-html`, e.g. `CornerFlourish.vue` and the player-board flourishes), so they are sanitized server-side in `server/svgsanitize.go` before storage/serving: `<script>`/`<foreignObject>`/`<style>` subtrees are dropped, `on*` and unsafe `href`/`src`/`xlink:href` attributes and dangerous SMIL animations (`<set>`/`<animate>` targeting `href`/`src`) are stripped, and `url()`/`@import` are removed from inline styles. The frontend also defensively sanitizes fetched SVG before `v-html` (`CornerFlourish.vue`), and theme-token values are injection-checked before they reach a `:root{...}` preview (`lib/theme-tokens.ts`).
