...
| `cards` | `id TEXT PK`, `board_data TEXT` (JSON 5×5 array), `player_name TEXT`, `details TEXT`, `world TEXT`, `protected INTEGER` (spared by Delete-All), `custom_status TEXT` (`''` normal / `pending` / `approved` — the custom-card-request lifecycle), `created_at` |
