...
func TestRegister_CreatesInactiveUserThatNeedsActivation(t *testing.T) {
	...
	resp = env.postJSON(t, "/api/users", map[string]any{"action": "set_active", "id": id, "active": true})
	resp.Body.Close()
...
func makeActiveUser(t *testing.T, env *testEnv, username, password string, perms []string) *http.Client {
	...
	resp = env.postJSON(t, "/api/users", map[string]any{"action": "set_active", "id": id, "active": true})
	resp.Body.Close()
	resp = env.postJSON(t, "/api/users", map[string]any{"action": "set_permissions", "id": id, "permissions": perms})
	resp.Body.Close()
...
func TestPermission_GrantedAllowedOthersForbidden(t *testing.T) {
	...
	// Granted page: generating cards works.
	resp := postAs(t, user, env, "/api/cards", map[string]any{"action": "generate", "count": 1})
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("granted cards action status = %d; want 200", resp.StatusCode)
	}
	resp.Body.Close()
...
func TestProtectedAdmin_CannotBeModifiedByAdmins(t *testing.T) {
	...
	for _, body := range []map[string]any{
		{"action": "set_active", "id": adminID, "active": false},
		{"action": "set_admin", "id": adminID, "admin": false},
		{"action": "set_password", "id": adminID, "password": "password123"},
		{"action": "delete", "id": adminID},
	} {
		resp := env.postJSON(t, "/api/users", body)
		if resp.StatusCode != http.StatusForbidden {
			t.Fatalf("action %v on admin: status = %d; want 403", body["action"], resp.StatusCode)
		}
		resp.Body.Close()
	}
}
...
func TestAccount_ChangePassword(t *testing.T) {
	...
	resp := env.postJSON(t, "/api/account", map[string]string{
		"action": "change_password", "current_password": "nope", "new_password": "password123",
	})
	if resp.StatusCode != http.StatusUnauthorized {
	...
	resp = env.postJSON(t, "/api/account", map[string]string{
		"action": "change_password", "current_password": seedAdminPass, "new_password": "newpassword1",
	})
	if resp.StatusCode != http.StatusOK {
